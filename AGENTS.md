...
- `ops/` — your cloned project repos (gitignored), e.g. `ops/infra/` — homelab
  infra-as-code (`WAAAGH/infra`): compose stacks + configs, secrets as `*.env.sops`,
  live Docker volumes/data untracked per its own `.gitignore`.
